...
if [ -n "$EXTERNAL_IP" ]; then
    export externalIp="$EXTERNAL_IP"
    echo "externalIp (from env): $externalIp"
else
    export externalIp="$(curl -s --max-time 5 https://ipinfo.io/ip)"
    echo "externalIp (from ipinfo.io): $externalIp"
fi 
echo "realm=$REALM" > /etc/turnserver.conf 
echo "no-udp=false" >> /etc/turnserver.conf
echo "no-tcp=false" >> /etc/turnserver.conf 
