# Legacy: Hubs Compose (upstream Hubs-Foundation)

> **About this document**
>
> This file preserves the original `README.md` from the upstream
> [Hubs-Foundation/hubs-compose](https://github.com/Hubs-Foundation/hubs-compose)
> repository, which this fork was based on. It targets the now-deprecated
...
> [`MANUAL_SETUP.md`](MANUAL_SETUP.md) instead. The content below is kept only
> for historical reference and for users who want to run vanilla upstream Hubs
> with `hubs-compose` (with the caveat that the upstream services were shut
> down at the end of May 2024).
...
- [Mutagen](https://mutagen.io/documentation/introduction/installation) (Linux note: installing the binaries manually in `/usr/local/bin` is probably the best route) The scripts start the Mutagen daemon, so it is _not_ necessary to configure your system to automatically start mutagen on boot.
...
> [!NOTE]
> Read [`README.md`](README.md) and [`MANUAL_SETUP.md`](MANUAL_SETUP.md) instead if you are using this project for **chutvrc**, not Hubs-Foundation. (This document was renamed from `START_DEVELOPMENT_FOR_CHUTVRC.md`.)
